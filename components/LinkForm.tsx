		...
		<div className="flex p-10 w-2/4 justify-center flex-col">
			...
			{variant === VARIANT_CREATE ? (
				...
				>
					<Input
						id="name"
						label="Link to simplify"
						onChange={(e: any) => { setForm({ ...form, originalUrl: e.target.value }) }}
						type="url"
						value={form.originalUrl}
						required
						placeholder="URL to be simplify e.g http://google.com" />
					...
					<div className="bg-black bg-opacity-20 p-6 rounded-md mt-3">
						...
						<div className="mt-3 flex">
							...
							<input
								...
								required
								minLength={2}
								className={`ml-2 rounded px-5 py-2 flex-1 focus:outline-none ${isSlugUsed ? 'border-2 border-red-500' : null}`} />
						</div>
					</div>
					<input
						type="submit"
						disabled={isSlugUsed}
						value="Simplify"
						className={`w-full mt-5 cursor-pointer rounded-md py-3 text-white text-xl bg-opacity-70 hover:bg-opacity-100 ${isSlugUsed ? 'bg-gray-300 cursor-not-allowed' : 'bg-blue-600'}`} />
				</form>
			) : (
				<div className="mt-5">
					<div 
					...
					<div className="flex gap-10 mt-5">
						<button 
							className="flex-1 bg-blue-600 bg-opacity-70 text-white rounded-md py-3 hover:bg-opacity-100"
							onClick={() => {
